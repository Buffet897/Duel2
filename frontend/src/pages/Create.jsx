...
const SlotUploader = ({ slot, label, onPick, preview, t }) => {
        ...
        {preview ? (
          <>
            ...
            <span className="absolute top-2 left-2 inline-flex items-center gap-1 bg-white/95 rounded-full px-2 py-1 text-[11px] font-semibold text-gray-800 shadow-sm">
              <RefreshCw className="h-3 w-3" /> {t("create.change")}
            </span>
          </>
        ) : (
          <>
            ...
            <span className="text-sm font-medium text-gray-600">
              {slot === "a" ? t("create.upload_a") : t("create.upload_b")}
            </span>
            <span className="text-xs text-gray-400 mt-1">{t("create.upload_hint")}</span>
          </>
        )}
...
const Create = () => {
  ...
  const placeholder = useMemo(() => {
    const examples = t("create.question_examples");
    if (Array.isArray(examples) && examples.length) {
      return examples[Math.floor(Date.now() / 5000) % examples.length];
    }
    return t("create.question_placeholder");
  }, [t]);
  ...
  const submit = async () => {
    if (!fileA || !fileB) {
      toast.error(t("errors.upload_missing"));
      return;
    }
    if (question.length > 80) {
      toast.error(t("errors.question_too_long"));
      return;
    }
    setSubmitting(true);
    try {
      ...
      toast.error(err?.response?.data?.detail || t("errors.create_failed"));
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <Shell>
      <div className="pt-2 pb-6">
        ...
        <p className="mt-2 text-sm text-gray-600">{t("create.subtitle")}</p>
      </div>

      <div className="grid grid-cols-2 gap-3" data-testid="upload-grid">
        <SlotUploader slot="a" label="A" preview={previewA} onPick={(f) => handlePick("a", f)} t={t} />
        <SlotUploader slot="b" label="B" preview={previewB} onPick={(f) => handlePick("b", f)} t={t} />
      </div>

      <div className="mt-6">
        <label className="text-xs font-semibold tracking-[0.05em] uppercase text-gray-500 block mb-2">
          {t("create.question_label")}
        </label>
        <input
          type="text"
          value={question}
        ...
        <div className="flex justify-between mt-1.5 text-[11px] text-gray-400">
          <span>{t("create.question_tip")}</span>
      ...
      <div className="mt-5 flex items-start gap-2 rounded-xl bg-[#F2F1FA] p-3 text-[12px] text-[#4A45A0] leading-snug">
        ...
        <span>{t("create.privacy_note")}</span>
      </div>
      ...
      >
        {submitting ? t("create.uploading") : (<>{t("create.submit")} <ArrowRight className="h-4 w-4" /></>)}
      </button>
    </Shell>
  );
};
