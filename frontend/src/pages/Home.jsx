    ...
    <Shell>
      <section className="pt-2 pb-8">
        ...
        <h1 className="text-3xl sm:text-4xl font-bold tracking-tight text-gray-950 leading-[1.1]">
          {t("home.h1_part1")}
          <br />
          <span className="text-[#7F77DD]">{t("home.h1_part2")}</span>
        </h1>
        <p className="mt-4 text-base text-gray-600 leading-relaxed">{t("home.subtitle")}</p>
        <Link
          to="/nieuw"
        ...
        >
          {t("home.cta_create")} <ArrowRight className="h-4 w-4" />
        </Link>
        <div className="mt-4 flex items-center justify-center gap-4 text-xs text-gray-500">
          <span className="flex items-center gap-1.5" data-testid="weekly-stats">
            <Users className="h-3.5 w-3.5" /> {stats.weekly.toLocaleString()} {t("home.active_duels")}
          </span>
          <span className="flex items-center gap-1.5">
            <Clock className="h-3.5 w-3.5" /> {t("home.duration")}
          </span>
        </div>
      </section>
      ...
      <section className="pt-4">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-display font-semibold text-gray-900">{t("home.cta_popular")}</h2>
          <span className="text-xs uppercase tracking-wider text-gray-400 font-semibold">Live</span>
        </div>
              ...
              <Wrapper key={d.id} {...props}>
                <div className="relative aspect-[3/4] rounded-xl overflow-hidden border border-gray-100 bg-gray-50">
                  <div className="grid grid-cols-2 h-full">
                    <img src={photoA} alt="A" loading="lazy" className="w-full h-full object-cover" />
                    <img src={photoB} alt="B" loading="lazy" className="w-full h-full object-cover border-l border-white" />
                  </div>
                  <div className="absolute inset-x-0 bottom-0 px-2 py-1.5 bg-gradient-to-t from-black/70 to-transparent text-[11px] text-white">
                    {total} {votesLabel(total, t)}
                  </div>
                </div>
                <p className="mt-2 text-sm text-gray-800 line-clamp-2 leading-snug">
                  {d.question || t("duel.default_question")}
                </p>
              </Wrapper>
            );
          })}
      ...
      <section className="mt-12 rounded-2xl bg-[#FAFAFA] border border-gray-100 p-5">
        <h3 className="font-display text-base font-semibold text-gray-900">{t("home.how_it_works")}</h3>
        <ol className="mt-3 space-y-2 text-sm text-gray-600">
          <li className="flex gap-3"><span className="text-[#7F77DD] font-bold">1.</span> {t("home.step_1")}</li>
          <li className="flex gap-3"><span className="text-[#7F77DD] font-bold">2.</span> {t("home.step_2")}</li>
          <li className="flex gap-3"><span className="text-[#7F77DD] font-bold">3.</span> {t("home.step_3")}</li>
        </ol>
      </section>
    </Shell>
  );
