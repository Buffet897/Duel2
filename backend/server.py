...
MAX_BYTES = 8 * 1024 * 1024  # cap raw upload at 8MB; we compress to ~2MB
...
async def compress_and_save(upload: UploadFile, slot: str, duel_id: str) -> str:
    ...
    if not raw:
        raise HTTPException(status_code=400, detail=f"Empty file for {slot}")
    if len(raw) > MAX_BYTES:
        raise HTTPException(status_code=400, detail=f"{slot} exceeds 8MB")

    try:
        image = Image.open(io.BytesIO(raw))
        image.load()
    except Exception as exc:
        raise HTTPException(status_code=400, detail=f"Invalid image for {slot}: {exc}")
...
async def popular_duels(limit: int = 6):
    ...
    async for doc in cursor:
        ...
        items.append(item)
    # sort by total votes desc, fall back to recency
    items.sort(key=lambda x: (-x["total"], x["created_at"]), reverse=False)
